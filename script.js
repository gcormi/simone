...
function showPage(pageNumber) {
    ...
    if (targetPage) {
        ...
        const audioPage11 = document.getElementById('audio-page-11'); // Musique épilogue principale (sonsdb.mp3)

        allAudios.forEach(aud => {
            if (aud.id !== `audio-page-${pageNumber}`) {
                if (!(aud.id === 'audio-page-11' && pageNumber === 12) && !aud.paused) {
                    aud.pause();
                    aud.currentTime = 0;
                }
            }
        ...
        if (pageNumber === 11) {
            if (audioPage11 && audioPage11.paused) {
                audioPage11.play().catch(error => console.warn(`Lecture auto pour audio-page-11 (sonsdb.mp3) bloquée:`, error));
            }
        } else if (pageNumber === 12) {
            if (audioPage11 && audioPage11.paused) { // Si on arrive sur la page 12 et que la musique de la page 11 est en pause
                 audioPage11.play().catch(error => console.warn(`Reprise lecture pour audio-page-11 (sonsdb.mp3) sur page 12 bloquée:`, error));
            }
        }
        ...
        setTimeout(() => {
            ...
            const staticLyricsCont = document.getElementById('static-lyrics-container');
            if (pageNumber === 12 && staticLyricsCont) {
                 staticLyricsCont.style.animation = `fadeInElement 0.4s ease-out ${pageContentElements.length * 0.05}s forwards`;
            }
        }, 10);
...
function startGame() {
    ...
    applyDyslexiaFontPreference(); // Appelé après que la page 0 (et son bouton DYS) est potentiellement affichée
    showPage(0);
}
...
function validerDefi1() {
    ...
    if (selectedOption) {
        ...
        document.getElementById('validateDefi1Btn').disabled = true;
...
// Défi 3: Destinée Biologique (QCM)
...
function initMotsMeles() {
    ...
    const motsListeSpan = document.getElementById('motsmeles-liste-complete');
    if (motsListeSpan) { // Normalement déjà dans l'HTML, mais on peut forcer ici
        motsListeSpan.textContent = motsMelesParams.words.join(', ');
    }
...
function placeWordInMotsMelesGrid(word) {
    ...
    let placed=false; const maxAttempts=100;
    for(let attempt=0; attempt<maxAttempts && !placed; attempt++){
        const dir=directions[Math.floor(Math.random()*directions.length)]; const len=normalizedWord.length;
        let r0,c0;
        if(dir.dr===0){r0=Math.floor(Math.random()*motsMelesParams.gridSize);c0=Math.floor(Math.random()*(motsMelesParams.gridSize-len+1));}
        else if(dir.dc===0){r0=Math.floor(Math.random()*(motsMelesParams.gridSize-len+1));c0=Math.floor(Math.random()*motsMelesParams.gridSize);}
        else if(dir.dc===1){r0=Math.floor(Math.random()*(motsMelesParams.gridSize-len+1));c0=Math.floor(Math.random()*(motsMelesParams.gridSize-len+1));}
        else {r0=Math.floor(Math.random()*(motsMelesParams.gridSize-len+1));c0=len-1+Math.floor(Math.random()*(motsMelesParams.gridSize-len+1));}
        ...
        for(let i=0;i<len;i++){
            ...
            if(motsMelesParams.grid[r][c]!==''&&motsMelesParams.grid[r][c]!==normalizedWord[i]){canPlace=false;break;}
        }
        if(canPlace){
            const currentWordObj={text:word,normalizedText:normalizedWord,cells:[],found:false};
            for(let i=0;i<len;i++){
                const r=r0+i*dir.dr; const c=c0+i*dir.dc;
                motsMelesParams.grid[r][c]=normalizedWord[i]; currentWordObj.cells.push({r,c});
            }
...
function validerSelectionMotsMeles() {
    ...
    const normalizedSelWord=selectedWordString; // Déjà normalisé car vient de la grille
    const normalizedSelWordRev=selectedWordString.split('').reverse().join('');
        ...
        if(!wordObj.found&&(wordObj.normalizedText===normalizedSelWord||wordObj.normalizedText===normalizedSelWordRev)){
            ...
            if(cellsMatch){
                for(let i=0;i<selectedPathCells.length;i++){
                    ...
                    const woCellForward = wordObj.cells[i];
                    const woCellBackward = wordObj.cells[selectedPathCells.length-1-i];
                    const isForwardMatch = (spCell.r === woCellForward.r && spCell.c === woCellForward.c);
                    const isBackwardMatch = (spCell.r === woCellBackward.r && spCell.c === woCellBackward.c);
                    if (wordObj.normalizedText === normalizedSelWord) {
                        if(!isForwardMatch) { cellsMatch = false; break; }
                    } else {
                         if(!isBackwardMatch) { cellsMatch = false; break; }
                    }
                }
            }
            if(cellsMatch){
...
function endMotsMelesGame(success) {
    ...
    if(success||actualFound>=motsMelesParams.targetWordsToFind){
        feedback.textContent=`Bravo ! Vous avez trouvé ${actualFound} mot(s). Défi réussi !`;
        feedback.className='feedback success'; handleSuccessfulChallenge(7);
    }else{
        feedback.textContent=(motsMelesParams.timeLeft<=0?`Temps écoulé ! `:` `) + `Vous avez trouvé ${actualFound} mot(s). Il en fallait ${motsMelesParams.targetWordsToFind} pour réussir.`;
...
function validerDefi8() {
    ...
    if (selectedOption) {
        ...
        document.getElementById('validateDefi8Btn').disabled = true;
        if (selectedOption.value === 'C') {
            feedback.textContent = "Excellente réponse ! La loi Veil de 1975 sur l'IVG, portée par Simone Veil alors Ministre de la Santé, fut une avancée majeure.";
            feedback.className = 'feedback success';
...
function validerDefi9() {
    ...
    inputElement.disabled = true;
    validerButton.disabled = true;
    const motsCles = ["action", "actions", "choix", "liberté", "existence", "actes", "faire", "agir"];
    if (motsCles.some(mot => reponse.includes(mot))) {
        feedback.textContent = "Très bien ! Simone de Beauvoir met l'accent sur notre capacité à nous définir par nos choix et nos actions.";
        feedback.className = 'feedback success';
        handleSuccessfulChallenge(9);
    } else {
        feedback.textContent = `Ce n'est pas tout à fait ça. Pensez à ce qui nous définit selon l'existentialisme (actions, choix, liberté...). La réponse attendue mettait en avant la capacité à agir ou les choix.`;
        feedback.className = 'feedback error';
...
function skipChallenge(challengeIndex) {
    const feedbackElementId = (challengeIndex === 7) ? `feedback-defi${challengeIndex}-motsmeles` : `feedback-defi${challengeIndex}`;
    const actualFeedbackElement = document.getElementById(feedbackElementId);
    ...
    if (pageElement) {
        if (challengeIndex === 1 || challengeIndex === 3 || challengeIndex === 8) {
             disableForm(`defi${challengeIndex}-form`);
    ...
    else if (challengeIndex === 7) message += "L'objectif était de trouver 4 mots parmi la liste.";
    else if (challengeIndex === 8) message += "La loi de 1975 concernait l'IVG, portée par Simone Veil.";
    else if (challengeIndex === 9) message += "La réponse attendue soulignait l'importance des actions ou des choix.";

    if (skippedWord) {
        message += ` L'indice qui aurait été révélé est : '${skippedWord}'`;
    } else {
        message += ` Aucun indice n'était directement lié à ce défi de cette manière.`;
    }
...
function initEventListeners() {
    document.getElementById('startGameBtn').addEventListener('click', () => showPage(1));
    ...
    if (returnToStartButton) {
        returnToStartButton.addEventListener('click', startGame);
    }

    const creditsModal = document.getElementById('credits-modal');
    const showCreditsModalBtn = document.getElementById('showCreditsModalBtn');
    const closeCreditsModalBtn = document.getElementById('closeCreditsModalBtn');

    if (showCreditsModalBtn && creditsModal && closeCreditsModalBtn) {
        showCreditsModalBtn.addEventListener('click', () => { creditsModal.style.display = 'block'; });
    ...
    window.addEventListener('click', (event) => {
        ...
        if (creditsModal && event.target === creditsModal) {
...
window.onload = () => {
    initEventListeners();
    startGame(); // Contient showPage(0) et applyDyslexiaFontPreference
};